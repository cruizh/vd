...
\begin{lemma}
  ...
  Toda aplicación \nameref{def:diferenciable} \(f \colon M \to N\) es continua
...
\begin{proof}\item
  ...
  \begin{subproof}[\(f(\mathcal{A})\) es compatible con \(\mathcal{B}\)]
    ...
    \Cref{lem:dif-implies-cont} y \(\Omega = U \cap f^{-1}(W)\) es abierto de
    ...
    Aquí usamos el \Cref{lem:dif-caract-exists}. Como \(\varphi_{0}(\Omega) =
...
\begin{note}
  La primera parte de la demostración del \Cref{lem:atlas-difeo} prueba que si tenemos
...
\begin{lemma}[lema de extensión]\label{lem:extension}
...
\begin{Answer}[number=16]
  ...
  \begin{center}
    \begin{tikzcd}
      \pi^{-1}(V_{p}) \ar[r, "\rho_{V_{p}}"] \ar[rd, "\pi"'] &
      V_{p}\times F \ar[d, "\pi_{1}"] \\
      & U
    \end{tikzcd}
  \end{center}
 ...
 \begin{center}
   \begin{tikzcd}
     V \ar[r,"\pi"] \ar[d,"\psi"] & W \ar[d,"\varphi"] \\
     \psi(V) \ar[r, dashed] & \varphi(W)
   \end{tikzcd}
 \end{center}
  ...
  \begin{center}
    \begin{tikzcd}
      \pi^{-1}(p) \ar[r, "\rho_{p}"] \ar[rd, "\pi"']
      & \Set{p} \times F \ar[d, "\pi_{1}"]\\
      & \Set{p}
    \end{tikzcd}
  \end{center}
