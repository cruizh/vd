...
\begin{definition}[\(\pderiv{\cdot}{x_{i}}\)]
Sea \(\pderiv{\cdot}{x_{i}}:\mathcal{F}(U)\to\mathcal{F}(U), 1\leq i\leq
m=\operatorname{dim}_{M}\), con \((U,\varphi)\) carta de \(M\), el operador tal
que si \(f\in\mathcal{F}(U),p\in U\) entonces:
\[
\Restrict{\pderiv{f}{x_{i}}}{p}=\Restrict{\pderiv{f\circ\varphi^{-1}}{x_{i}}}{\varphi(p)}\in\mathbb{R}
\]
\end{definition}

\begin{lemma}
  \(\pderiv{}{x_{i}}\) está bien definida \(\leftrightarrow\)
  \(\pderiv{f}{x_{i}}\) es diferenciable \(\forall f\in\mathcal{F}(U)\)
\end{lemma}
\begin{proof}
\end{proof}

\begin{lemma}
\(\forall f\in\mathcal{F}(U\cap V)\), con \((U,\varphi),(V,\psi)\) cartas de
\(M\) con \(U\cap V\neq\emptyset\), \(x_{i},y_{i}\equiv f\) coordenadas de cada
carta respectivamente, entonces:
\[\Restrict{\pderiv{f}{y_{i}}}{p}=\sum_{j=1}^{m}\Restrict{\pderiv{f}{x_{j}}}{p}\cdot\Restrict{\pderiv{(\varphi\circ\psi^{-1})_{j}}{y_{i}}}{\psi(p)}\]
\end{lemma}
\begin{proof}
...
\begin{lemma}
La función \[\alpha:\mathbb{R}\to\mathbb{R}\]
\[\alpha(t) =
\begin{dcases}
0 & t \leq 0 \\
e^{-1/t} & t > 0
...
\begin{lemma}
  \(\exists\lambda:\mathbb{R}^{n}\to\mathbb{R}\) diferenciable tal que \[\lambda(x) =
\begin{dcases}
  1 & \Norm{x} \leq 1 \\
