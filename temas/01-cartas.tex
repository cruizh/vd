...
\begin{proof}
  ...
  \begin{subproof}[\(\psi' \circ \psi^{-1}\) diferenciable]
    ...
    \begin{figure}[h]
      \centering
      \begin{tikzcd}
        \psi(U \cap W \cap W') \arrow[rr, "\psi' \circ \psi^{-1}"] \arrow[dr,
        "\varphi \circ \psi^{-1}"']&&
        \psi'(U \cap W \cap W')\\
        &\varphi(U \cap W \cap W') \arrow[ur, "\psi' \circ \varphi^{-1}"']&
      \end{tikzcd}
    \end{figure}
...
\begin{definition}[name={variedad},label={def:vd}]
  ...
  un \nameref{def:atlas} numerable \(\mathcal{A}\) formado por
...
\begin{definition}[name={DIF-topología},label={def:diftop}]
  Llamaremos \emph{DIF-topología} de \(M^{m}\) a la \nameref{def:topnat} de un
  atlas \(\mathcal{A}\) de \(M\).
\end{definition}

\begin{lemma}
  \(\mathcal{T}_{\mathcal{A}}\) es topología sobre \(M\).
\end{lemma}

\begin{proof}\item
  \begin{subproof}[\(\emptyset, M \in \mathcal{T}_{\mathcal{A}}\)]
    Trivial.
  \end{subproof}
...
\begin{lemma}[name={caracterización de abiertos en topología natural}, label={lem:topnat-caract}]
  Son equivalentes:
  \begin{gather}
  ...
  En particular \(\mathcal{T}_{\mathcal{A}} = \mathcal{T}_{\mathcal{B}}\) si
  \(\mathcal{A}\) y \(\mathcal{B}\) son atlas compatibles.
\end{lemma}
...
\begin{proof}\item
  ...
  \begin{subproof}[\(\ref{eq:topnat-caract-1} \implies \ref{eq:topnat-caract-2}\)]
    ...
    abierto de \(\RealSet^{m}\). Por hipótesis,  \(\varphi(G \cap U)\) es
    abierto de \(\varphi(U) \subseteq \RealSet^{n}\) y por tanto, \(\varphi(G
    \cap U) \cap \varphi(U \cap W) = \varphi(G \cap U \cap W)\) es abierto.

    % TODO: Consultar subproof a partir de aquí
    Además, \(\psi \circ \varphi^{-1}\) es difeomorfismo, por lo que es
    homeomorfismo y por tanto \(\psi \circ \varphi^{-1} (\varphi(G \cap U \cap
    W)) = \psi(G \cap U \cap W)\) es abierto de \(\psi(W) \subseteq
    \RealSet^{m}\) conteniendo a \(x\). Luego \(\psi(G \cap W)\) es abierto de \(\RealSet^{m}\)
  \end{subproof}
  ...
  \begin{subproof}[\(\ref{eq:topnat-caract-2} \implies \ref{eq:topnat-caract-3}\)]
    ...
    Sea \(V = U \cap G\). Tenemos así que la carta \((V, \rho) = \left( U \cap G,
    \Restrict{\varphi}{U \cap G} \right)\) es compatible con \(\mathcal{A}\)
...
\begin{definition}[name={2ºN}, label={def:2n}]
  ...
  que es \emph{2º-numerable} si existe una \nameref{def:base-topo} numerable de la topología.
\end{definition}
...
\begin{proof}
  Sea \(\mathcal{B}\) \nameref{def:base-topo} numerable de \((X,\mathcal{T})\). Entonces
