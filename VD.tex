\usepackage{polyglossia}
\setdefaultlanguage{spanish}

\usepackage{subfiles}
\usepackage{xr}

\usepackage{amsmath,amssymb,mathtools}
\usepackage{xfrac}
\usepackage{amsthm,thmtools}
\usepackage{xpatch}
\renewcommand{\listtheoremname}{Lista de Teoremas}
\xpatchcmd{\listoftheorems}{\@fileswfalse}{}{\typeout{yes}}{\typeout{no}}

\usepackage{graphicx} 
\graphicspath{{\main/figures/}}

\usepackage{faktor} % Espacios cociente
\usepackage{exercise}
\usepackage{cool}
\usepackage[inline]{enumitem}
\usepackage{subcaption}
\usepackage{mathrsfs}

\usepackage{tikz, tikz-cd}
% \usepackage{pgfplots}
\usetikzlibrary{decorations.markings,arrows, shapes.geometric}

% Permite añadir símbolos de relación centrados en lugar de flechas
\tikzset{
  symbol/.style={
    draw=none,
    every to/.append style={
      edge node={node [sloped, allow upside down, auto=false]{$#1$}}}
  }
}

% Elimina pruebas y soluciones de ejercicios si estamos generando el documento
% sin pruebas
\ifdefined\noproof
\usepackage{versions}
\excludeversion{proof}
\excludeversion{Answer}
\fi

\usepackage[unicode,breaklinks=true]{hyperref}
\usepackage{nameref,cleveref}

\usepackage{xcolor}
\hypersetup{
    colorlinks,
    linkcolor={red!50!black},
    citecolor={blue!50!black},
    urlcolor={blue!80!black}
}

\newcommand{\nphantom}[1]{\sbox0{#1}\hspace{-\the\wd0}}

% rhcool
\newcommand{\NaturalSet}{\ensuremath{\mathbb{N}}}   % Naturales
\newcommand{\IntegerSet}{\ensuremath{\mathbb{Z}}}   % Enteros
\newcommand{\RationalSet}{\ensuremath{\mathbb{Q}}}  % Racionales
\newcommand{\RealSet}{\ensuremath{\mathbb{R}}}      % Reales
\newcommand{\ComplexSet}{\ensuremath{\mathbb{C}}} % Complejos
\newcommand{\Parts}[1]{\ensuremath{\mathcal{P}(#1)}} % Partes de un conjunto
\newcommand{\Field}{\ensuremath{\mathbb{K}}}        % Cuerpo arbitrario
\newcommand{\ProjectiveSpace}{\ensuremath{\mathbb{P}}} % Espacio proyectivo
\DeclarePairedDelimiter{\Norm}{\lVert}{\rVert}
\makeatletter
\newcommand{\Set}[2][]{%
\listval{#1}{0}% Record length of list
\setcounter{COOL@listlen}{\value{COOL@listpointer}}
\ifthenelse{\value{COOL@listlen} = 0}{%
  \{#2\}%
}{% else
  \ifthenelse{\value{COOL@listlen} = 1}{%
    \{ #2 \}_{#1}%
  }{% else
    \ifthenelse{\value{COOL@listlen} = 3}{%
      \{ #2 \}_{\listval{#1}{1} = \listval{#1}{2}}^{\listval{#1}{3}}%
    }{% else
      \PackageError{cool}{Invalid list length for ‘Set’}%
      {can only have none, one or three elements for second argument}%
    }%
  }%
}%
}
\makeatother


% rhtopology
\makeatletter
\newcommand{\COOL@notation@InteriorParen}{p}
\newcommand{\Interior}[1]{\operatorname{int}\COOL@decide@paren{Interior}{#1}}
\newcommand{\Exterior}[1]{\operatorname{ext}\COOL@decide@paren{Interior}{#1}}
\newcommand{\Clausura}[2][]{\overline{#2}^{#1}}
\newcommand{\Frontera}[1]{\partial{}#1}
\newcommand{\Dist}[2]{\operatorname{d}\left( #1,\, #2 \right)}
\newcommand{\Bola}[2]{\operatorname{B}\left( #1,\, #2 \right)}
\newcommand{\Id}[1]{\mathbf{1}_{#1}}
\newcommand{\Restrict}[2]{{% we make the whole thing an ordinary symbol
  \left.\kern-\nulldelimiterspace{}% automatically resize the bar with \right
  #1 % the function
  \vphantom{\big|} % pretend it's a little taller at normal size
  \right|_{#2} % this is the delimiter
}} % Source: https://tex.stackexchange.com/questions/22252
\newcommand{\T}[1]{T_{#1}} % Clases de espacio topológico
\newcommand{\sumacon}{\mathop{\#}} % Suma conexa
\newcommand{\homeo}{\cong} % Homeomorfo
\newcommand{\homotop}{\simeq} % Homotópico
\newcommand{\Matrices}[3]{\operatorname{\mathcal{M}_{#1 \times #2}} \left( #3 \right)}
\newcommand{\GL}[2]{\operatorname{GL}\left(#1,\, #2\right)} % Grupo lineal
\newcommand{\COOL@notation@GFParen}{p}
\newcommand{\GF}[1]{\operatorname{\pi_{1}}\COOL@decide@paren{GF}{#1}} % Grupo
% fundamental
\newcommand{\Free}[1]{\operatorname{F}\left(#1\right)}
\newcommand{\normal}{\lhd} % Subgrupo normal
\DeclarePairedDelimiter{\Generado}{\langle}{\rangle} % Atlas maximal generado
\DeclarePairedDelimiter{\SG}{\langle}{\rangle} % Subgrupo generado
\DeclarePairedDelimiter{\SGN}{\lhd}{\rhd} % Subgrupo normal generado
\DeclareMathOperator{\Ker}{Ker}
\newcommand{\Fdif}[1]{\operatorname{\mathcal{F}}\left(#1\right)} % Funciones diferenciables.
\DeclareMathOperator{\rango}{rango}
\newcommand{\Clase}[1]{\left[#1\right]} % Clase de equivalencia
\newcommand{\Tangente}[2][]{\operatorname{T}_{#1} {#2}} % Espacio tangente
\newcommand{\DualT}[2][]{\operatorname{T}^{*}_{#1} {#2}} % Espacio dual tangente
\newcommand{\ProdC}[2]{\left[#1,\, #2 \right]} % Producto corchete
\makeatother

% rhanalysis
\newcommand{\J}[2][]{\operatorname{J}_{#1} ({#2})} % Matriz jacobiana
\newcommand{\dif}[2][]{\operatorname{d}_{#1}\!{#2}} % Diferencial

% thmrh

\declaretheorem[
name=Teorema,
refname={teorema,teoremas},
numberwithin=chapter%
]{theorem}
  
\declaretheorem[
name=Proposición,
refname={proposición,proposiciones},
sibling=theorem
]{proposition}

\declaretheorem[
name=Propiedad,
refname={propiedad,propiedades},
sibling=theorem
]{property}
  
\declaretheorem[
name=Ejemplo,
refname={ejemplo,ejemplos},
sibling=theorem
]{example}

\declaretheorem[
name=Corolario,
refname={corolario,corolarios},
sibling=theorem
]{corollary}

\declaretheorem[
name=Criterio,
refname={criterio,criterios},
sibling=theorem
]{criterion}

\declaretheorem[
name=Lema,
refname={lema,lemas},
sibling=theorem
]{lemma}

\declaretheorem[
name=Definición,
style=definition,
refname={definición,definiciones},
sibling=theorem
]{definition}

\declaretheorem[
name=Observación,
style=remark,
refname={observación,observaciones},
sibling=theorem
]{remark}
  
\declaretheorem[
name=Nota,
style=remark,
refname={nota,notas},
sibling=theorem
]{note}

% \numberwithin{equation}{theorem}

\newenvironment{subproof}[1][\proofname]{%
  \renewcommand{\qedsymbol}{$\blacksquare$}%
  \begin{proof}[#1]%
}{%
  \end{proof}%
}

\title{Variedades Diferenciables}
\author{%
  Quintero Toscano, Antonio Rafael \\
  Camacho Moro, Jesús \\
  Gutiérrez Camacho, Adrián \\
  Ruiz-Henestrosa Ruiz, Carlos José}
\date{\today}

\begin{document}

\begin{titlingpage}
  \maketitle
\end{titlingpage}

\tableofcontents

\clearpage

\listoftheorems[ignore={example,exercise}]

\subfile{temas/00-intro}

\subfile{temas/01-cartas}

\subfile{temas/02-aplicaciones}

\subfile{temas/03-subvariedades}

\subfile{temas/04-tangente}

\subfile{temas/05-diferencial}

\subfile{temas/06-campos}

\subfile{temas/07-campos2}

\subfile{temas/08-algebralie}

\subfile{temas/09-cotangente}

\subfile{temas/10-tensores}

\subfile{temas/11-curvas-integrales}

\end{document}

%%% Local Variables:
%%% mode: latex
%%% mode: flyspell
%%% mode: outline
%%% ispell-local-dictionary: "castellano"
%%% End: